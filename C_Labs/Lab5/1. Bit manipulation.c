...
True means that a binary number was successfully read. Call read_line to read strings that are to be converted to unsigned integers. 
...
bool binary_reader(unsigned int *pu){
    *pu = 0;
    ...
    if(read_line(str, 50)){
        unsigned char *binary = strstr(str, "0b");
        
        if(binary){
            if(binary[strlen(binary) - 1] == '\n'){
                binary[strlen(binary) - 1] = '\0';
            }
            for(int i = 2; binary[i] == '0' || binary[i] == '1'; i++){
                *pu <<= 1;
                *pu |= (binary[i] - '0');
            }
            return 1;
